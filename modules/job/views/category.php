...
if($get->num_rows()){
    foreach($get->result() as $row){
        echo '<tr data-id="'.$row->job_category_id.'">
            <td>'.$row->title.'</td>
            <td>'.$this->ki_theme->drawer_button('job_category',$row->job_category_id,$row->title).'</td>
            <td>
            <div class="btn-group">
                <buttons class="btn btn-primary btn-xs btn-sm edit-record"><i class="ki-outline ki-pencil"></i></buttons>
                <button class="btn btn-danger btn-xs btn-sm delete-record"><i class="fa fa-trash"></i></button>
            </div></td>
...
<script id="formTemplate" type="text/x-handlebars-template">
    <input type="hidden" name="id" value="{{job_category_id}}">
    <div class="form-group">
